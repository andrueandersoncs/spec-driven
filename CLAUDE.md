...
Projects can include `.claude/spec-driven.local.md` for project-specific settings:
...
Any markdown content here provides additional context for the plugin.
```

**Configuration Fields:**
- `archetype`: Software type (cli, library, web-service, data-pipeline, desktop-app, compiler)
- `domains`: Applicable domains (finance, healthcare, auth, multi-tenant, e-commerce, real-time)
- `preferences.autoVerify`: Run verification automatically after spec changes
- `preferences.generateTests`: Generate tests during code generation
- `preferences.targetLanguage`: Target language for generation (currently: typescript)
